...
namespace aft
{
  ...
  public:
      ...
      const std::unordered_set<ccf::NodeId>& new_learners = {})
    {
      ...
      {
        for (auto& [nid, info] : conf)
        {
          if (nid != state->my_node_id)
          {
            LOG_DEBUG_FMT(
              "Configurations: update node information for {}", nid);
            auto nit = nodes.find(nid);
            if (nit == nodes.end())
            {
              nodes[nid] = NodeState(info, state->last_idx + 1, 0);
            }
            else
            {
              nit->second.node_info = info;
            }
          }
        }
    ...
    void reconfigure(ccf::SeqNo seqno, const kv::NetworkConfiguration& config)
    {
      LOG_DEBUG_FMT("Configurations: reconfigure to {{{}}}", config);
      std::unique_lock<std::mutex> guard(state->lock, std::defer_lock);
      ...
      network_configurations[config.rid] = config;
      orc_sets[config.rid] = {};

      Configuration::Nodes conf_nodes;
      for (const auto& nid : config.nodes)
      {
        if (nid == state->my_node_id)
        {
          conf_nodes[nid] = {};
        ...
        {
          auto iit = nodes.find(nid);
          assert(iit != nodes.end());
          conf_nodes[nid] = iit->second.node_info;
        }
      }
      uint32_t offset = get_bft_offset(conf_nodes);
      configurations.push_back(
        {seqno, std::move(conf_nodes), offset, config.rid});
    ...
    bool orc(kv::ReconfigurationId rid, const NodeId& node_id)
    {
      ...
      auto oit = orc_sets.find(rid);
      auto ncit = network_configurations.find(rid);

      if (oit == orc_sets.end() || ncit == network_configurations.end())
      {
        return false;
      }
